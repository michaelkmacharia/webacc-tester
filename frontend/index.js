...
const download = ( async ( request ) =>
	{
		...
		const csv = accessibility_issues . map ( ( accessibility_issue ) =>
			{
		...
		csv_link . download = `webacc-tester-results-` + csv_url . substring ( 12 ) + `.csv` ;
...
const show = ( ( accessibility_issues ) =>
	{
		...
		accessibility_issues . forEach ( ( accessibility_issue ) =>
			{
				let output = `<div class="card mb-3 rounded"><div class="card-body"><h5>${ accessibility_issue . message }</h5><p class="bg-light p-3 my-3">${ escape ( accessibility_issue . context ) }</p><p class="bg-secondary text-light p-2 rounded">CODE: ${ accessibility_issue . code }</p></div></div>` ;
